...
class PidLineFollowerConfig:
    """PID 循线器参数。"""
    control_timestep_s: float = 0.01
    gain: float = 28.0
    integral_gain: float = 12.0
    derivative_gain: float = 2.4
    turn_limit_rps: float = 2.8
    min_speed_mps: float = 0.09
    integral_limit_m: float = 0.12
    reacquire_speed_mps: float = 0.08
    reacquire_turn_rate_rps: float = 1.6

class PidLineFollower(BaseLineFollower):
    """基于横向误差的 PID 循线器。"""
    def __init__(self, config: PidLineFollowerConfig | None = None) -> None:
        self.config = config or PidLineFollowerConfig()
    ...
    def command(
        ...
        if line_reading.line_detected and line_reading.lateral_error_m is not None:
            ...
            self.integral_error_m = float(
                np.clip(
                    self.integral_error_m + error * self.config.control_timestep_s,
                    -self.config.integral_limit_m,
                    self.config.integral_limit_m,
                )
            )
            ...
            angular_speed = float(
                np.clip(
                    ...
                    + self.config.integral_gain * self.integral_error_m
